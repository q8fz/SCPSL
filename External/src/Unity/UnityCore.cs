// DMA Base - Unity IL2CPP object layouts (GameObject, components, GOM helpers).

using DmaBase.DMA;
using DmaBase.Misc;
using DmaBase.Misc.Pools;
using DmaBase.Unity.IL2CPP;
...
{
    /// <summary>djb2-style hash used by some Unity runtime string tables.</summary>
    ...
    /// <summary>IL2CPP GameObject layout — name, components, and class lookup.</summary>
    ...
    public readonly struct GameObject
    {
        ...
        public string GetName(int maxLen = 128, bool useCache = true)
        {
            ...
            return DmaMemory.ReadString(NamePtr, maxLen, useCache) ?? string.Empty;
        }
        ...
        public ulong GetComponent(string className, bool useCache = true)
        {
            ...
            try
            {
                DmaMemory.ReadBuffer(componentArr.ArrayBase, entries.AsSpan());
            }
            catch
            {
                return 0;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                try
                {
                    name = global::DmaBase.Unity.ObjectClass.ReadName(objectClass, 128, useCache);
                }
                catch
                {
                    continue;
        ...
        public static ulong GetComponent(ulong gameObjectPtr, string className, bool useCache = true)
        {
            ...
            var go = DmaMemory.ReadValue<GameObject>(gameObjectPtr, useCache);
    ...
    public readonly struct MonoBehaviour // Behaviour : Component : EditorExtension : Object
    {
        ...
        public readonly GameObject GetGameObject() =>
            DmaMemory.ReadValue<GameObject>(ObjectClass);
        ...
        {
            var go = DmaMemory.ReadPtr(behaviour + GameObjectOffset);
            return 0;
        }
    }
    [StructLayout(LayoutKind.Sequential, Pack = 1)]
    ...
    /// <summary>Managed object class header — use to resolve native type names.</summary>
    public readonly struct ObjectClass
    {
        ...
        {
            var namePtr = DmaMemory.ReadPtrChain(objectClass, To_NamePtr, useCache);
            return DmaMemory.ReadString(namePtr, length, useCache);
        }
            ...
            int maxLen = 64)
        {
            ...
            ulong klass = DmaMemory.ReadPtr(objectBase, false);
